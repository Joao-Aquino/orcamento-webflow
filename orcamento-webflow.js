...
(() => {
  ...
  function clearCartList() {
    const cartList = Array.from(
      document.getElementsByClassName("nav-cart_list")
    )[0];
  ...
  function onRequestFormSubmit() {
    const button = Array.from(
      document.getElementsByClassName("nav-cart_list-form-button")
    )[0];
  ...
  function setUpCartFromLocalStorage() {
    clearCartList();
    const quoteItems = JSON.parse(localStorage?.getItem("quoteItems") || "{}");
    let allItemsString = "";
    ...
    Object.keys(quoteItems).forEach(function (key) {
      ...
      // Concatenar itens na string
      allItemsString += `Item: ${quoteItems[key]["slug"]} - ${quoteItems[key]["quantity"]}, `;
    });

    // Remover a última vírgula e espaço
    allItemsString = allItemsString.slice(0, -2);

    // Adicionar a string concatenada a um único input escondido
    const hiddenInput = document.createElement("input");
    hiddenInput.type = "hidden";
    hiddenInput.name = "Produtos para orçamento";
    hiddenInput.value = allItemsString;
    hiddenInput.className = "quote-item";
    document.getElementById("wf-form-Request-Quote")?.appendChild(hiddenInput);
  ...
  function addClickEventListenerToNavCartOpen() {
    const cartLink = Array.from(
      document.getElementsByClassName("nav-cart_button-wrapper")
    )[0];
